...
// memory cache Redis connection address
...
// auto detect best Redis connection
async function detectBestRedisUrl() {
  ...
  for (const url of possibleUrls) {
    try {
      ...
      const testClient = createClient({ url: url as string });
      testClient.on('error', () => {});
...
export const cachePool = createPool(
  {
    create: async () => {
      ...
      try {
        ...
        // @ts-ignore add del method
        // memoryCache.del = memoryCache.delete;
        // cache = memoryCache;
      }
